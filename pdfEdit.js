...
async function convertGoogleDriveLink(shareUrl) {
    // Извлекаем ID файла из ссылки
    const match = shareUrl.match(/\/d\/([a-zA-Z0-9_-]+)/);
    
    if (!match || !match[1]) {
        throw new Error('Неверный формат ссылки Google Диска');
    }
...
function formatNumber(num) {
  return num.toFixed(2) // округляем до 2 знаков после запятой
    .replace('.', ',')  // заменяем точку на запятую (десятичный разделитель)
...
async function add_blue_circle_and_line_with_name_table(page, custom_bold_font, table_data, custom_font, newTableCordinates, moveDistance) {
    ...
    console.log(`x: ${amount_liabilities_data.x}`);
    console.log(`y: ${amount_liabilities_data.y}`);
    console.log(`width: ${amount_liabilities_data.width}`);
    console.log(`height: ${amount_liabilities_data.height}`);

    await add_amount_in_box(
        page,
        remaining_balance_data.rectangleArea,
        table_data.remaining_balance,
    ...
    await add_amount_in_box(
        ...
        table_data.remaining_balance * 12,
...
async function moveContentDown(page, tableArea, newTableArea, moveDistance, pdfDoc) {
...
async function drawTable(page, tableData, x, startY, custom_font, custom_bold_font, tableTypeData, tableCoordinates, newTableCoordinates) {
    ...
    page.drawRectangle({
        x: x,
        y: currentY - headerRowHeight, // рисуем вниз от startY
    ...
    headers.forEach((header, colIndex) => {
        ...
        lines.forEach((line, lineIndex) => {
            const textWidth = custom_bold_font.widthOfTextAtSize(line, 12);
            const textX = currentX + (colWidths[colIndex] - textWidth) / 2;
            
            // Вертикальное центрирование для многострочного текста
            const totalTextHeight = lines.length * lineHeight;
            const textY = currentY - headerRowHeight + (headerRowHeight - totalTextHeight) / 2 + (lines.length - lineIndex - 1) * lineHeight;
    ...
    tableData.forEach((row, rowIndex) => {
        ...
        headers.forEach((header, colIndex) => {
            ...
            if (header === 'Ежемесячный платеж' || header === 'Общая сумма обязательства') {
    ...
    if (tableTypeData.type == "liability") {
        await add_blue_circle_and_line_with_name_table(page, custom_bold_font, tableTypeData, custom_font, newTableCoordinates, tableData.length * 15);
    };
...
async function replaceTableInPDF(page, newTableData, tableCoordinates, tableData, pdfDoc, custom_bold_font, custom_font) {
    ...
    //await eraseTableArea(page, tableCoordinates);
    ...
    await moveContentDown(page, tableCoordinates, newTableCoordinates, height_difference, pdfDoc);
    
    // Рисование новой таблицы
    await drawTable(page, newTableData, tableCoordinates.x, (tableCoordinates.y2 - tableCoordinates.height) + 26, custom_font, custom_bold_font, tableData, { ...tableCoordinates, height: newHeightTable, y: newTableY}, newTableCoordinates);
};

async function main_function(data) {
    // Скачиваем PDF документ.
    const url = await convertGoogleDriveLink("https://drive.google.com/file/d/1lpBwzo9uyanqyXKeNsFDVXi26chs2JrO/view?usp=sharing")
    console.log(url);
    const existingPdfBytes = await fetch(url).then(res => res.arrayBuffer());


    // Скачиваем шриафты.
    const url_font = "https://db.onlinewebfonts.com/t/629a55a7e793da068dc580d184cc0e31.ttf";
    const fontBytes = await fetch(url_font).then(res => res.arrayBuffer());

    const url_bold_font = "https://db.onlinewebfonts.com/t/50145685042b4df07a1fd19957275b81.ttf";
    const fontBoldBytes = await fetch(url_bold_font).then(res => res.arrayBuffer());
    ...
    // Считаем индекс.
    const totalIncome = data.sources_of_official_income.reduce((acc, item) => acc + item["Сумма в месяц, руб."], 0);
    ...
    await replaceTableInPDF(pages[2], data.liabilities, liabliityTableArea, data, pdfDoc, custom_bold_font, custom_font);

    // Сохраняем PDF.
    const pdfBytes = await pdfDoc.save();
    return pdfBytes;
    /*
    fs.writeFileSync("govno.pdf", pdfBytes);
